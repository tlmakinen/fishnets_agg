...
def fill_diagonal_torch(a, val):
    a_ = a.clone()
    a_[..., torch.arange(0, a_.shape[0]), torch.arange(0, a_.shape[0])] = val
...
class FishnetsAggregation(Aggregation):
    ...
    def __init__(self, n_p: int, in_size: int = None, semi_grad: bool = False, 
        ...
        self.lin_1 = Linear(in_size, fdim, bias=True).to(self.device)
        self.lin_2 = Linear(n_p, in_size, bias=True).to(self.device)
    ...
    def forward(self, x: Tensor, index: Optional[Tensor] = None,
        ...
        x = self.lin_1(x.to(self.device))
        
        # CONSTRUCT SCORE AND FISHER
        # the input x will be n_p + (n_p*(n_p + 1) // 2) long
        score = x[..., :self.n_p].to(self.device)
        fisher = x[..., self.n_p:].to(self.device)
        ...
        # add in the prior 
        fisher = fisher + torch.eye(self.n_p).to(self.device)
